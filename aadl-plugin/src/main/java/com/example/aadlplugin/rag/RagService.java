...
public class RagService {
    ...
    public SearchResult search(String query, String agentType) {
        ...
        log.info(String.format("Starting RAG search for query (len=%d, agentType=%s): %s", 
        ...
        log.info(String.format("Rewritten query (len=%d): %s", 
        ...
        if (qdrantVectorStore.isAvailable()) {
            ...
            if (results == null || results.isEmpty()) {
                log.warning("Qdrant search returned empty results, falling back to memory search");
        ...
        log.info(String.format("RAG search completed in %dms (agentType: %s, qdrant: %s)", searchTime, agentType, qdrantVectorStore.isAvailable()));
    ...
    private List<Document> searchWithQdrant(String query, String agentType) {
        try {
            log.fine(String.format("Generating embedding for query (len=%d, agentType=%s)", 
                    query != null ? query.length() : -1, agentType));
            
            float[] queryEmbedding = embeddingService.embed(query);
            
            log.info(String.format("Embedding generation result: null=%s, length=%d", 
                    queryEmbedding == null, 
                    queryEmbedding != null ? queryEmbedding.length : -1));
            
            if (queryEmbedding != null && queryEmbedding.length > 0) {
                log.fine(String.format("Generated query embedding with dimension: %d", queryEmbedding.length));
                ...
                log.warning("Failed to generate valid embedding for query (null or empty), falling back to keyword search");
                return qdrantVectorStore.keywordSearch(agentType, query, ragConfig.getTopK());
            }
        } catch (Exception e) {
            log.severe(String.format("Qdrant search failed: %s, falling back to memory search", e.getMessage()));
            return null;
        }
    }
    ...
    private List<Document> searchInMemory(String query, String agentType) {
        try {
            ...
            log.severe(String.format("Memory search failed: %s", e.getMessage()));
